...
const Select = ({ value, options, onChange }: { value: string; options: string[]; onChange: (v: string) => void }) =>
  IS_WEB ? (
    <select value={value} onChange={(e: any) => onChange(e.target.value)} style={{
      ...
      outline: 'none', cursor: 'pointer', color: '#2F3349',
    } as any}>
...
const DateInput = ({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) =>
  IS_WEB ? (
    <input type="date" value={value} onChange={(e: any) => onChange(e.target.value)} style={{
      width: '100%', padding: '9px 12px', fontSize: '13px', fontFamily: 'Inter, sans-serif',
      border: '1px solid #DBDADE', borderRadius: '8px', background: '#FAFAFE',
      outline: 'none', boxSizing: 'border-box', color: '#2F3349',
    } as any} />
  ) : (
  ...
  const content = (
    <View style={s.sheet}>
      ...
      <View style={s.header}>
        ...
        <View style={s.pips}>
          {[1,2,3].map(i => (
            <View key={i} style={[s.pip, { backgroundColor: i <= progress ? '#7367F0' : '#DBDADE', width: i <= progress ? 28 : 16 }]} />
          ))}
        </View>
        {onClose && (
      ...
      <View style={s.authRow}>
        {authUser ? (
          ...
          <Text style={{ fontSize: 13, fontFamily: 'Inter_400Regular', color: '#8588A5' }}>
      ...
      <View style={s.footer}>
        ...
        <TouchableOpacity
          style={[s.footerSave, { backgroundColor: saved ? '#28C76F' : '#7367F0' }]}
...
const s = StyleSheet.create({
  ...
  avatar:     { width: 30, height: 30, borderRadius: 15, backgroundColor: '#7367F0', alignItems: 'center', justifyContent: 'center' },
  avatarTxt:  { fontSize: 13, fontFamily: 'Inter_700Bold', color: '#fff' },
  authEmail:  { fontSize: 13, fontFamily: 'Inter_600SemiBold', color: '#2F3349' },
  authPlan:   { fontSize: 10, fontFamily: 'Inter_400Regular', color: '#8588A5', marginTop: 1 },
  ...
  tabIcon:    { fontSize: 13 },
  tabTxt:     { fontSize: 12, fontFamily: 'Inter_500Medium', color: '#8588A5' },
  ...
  flabel:     { fontSize: 11, fontFamily: 'Inter_600SemiBold', color: '#8588A5', letterSpacing: 0.3 },
  fhint:      { fontSize: 10, fontFamily: 'Inter_400Regular', color: '#ACAEC5' },
  input:      { height: 38, backgroundColor: '#FAFAFE', borderWidth: 1, borderColor: '#DBDADE', borderRadius: 8, paddingHorizontal: 12, fontSize: 13, fontFamily: 'Inter_400Regular', color: '#2F3349' },
  chip:       { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 6, backgroundColor: '#F4F5FA', borderWidth: 1, borderColor: '#DBDADE' },
  chipOn:     { backgroundColor: '#F0EEFF', borderColor: '#7367F0' },
  chipTxt:    { fontSize: 11, fontFamily: 'Inter_500Medium', color: '#8588A5' },
  ...
  footerBack: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 10, paddingHorizontal: 14, borderRadius: 8, backgroundColor: '#F4F5FA', borderWidth: 1, borderColor: '#DBDADE' },
  footerBackTxt:{ fontSize: 13, fontFamily: 'Inter_500Medium', color: '#8588A5' },
